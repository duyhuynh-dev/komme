...
@inngest_client.create_function(
    fn_id="pulse-scheduled-weekly-digests",
    trigger=inngest.TriggerCron(cron="TZ=America/New_York */15 * * * *"),
)
async def weekly_recommendations(_: inngest.Context) -> dict[str, str]:
    result = await trigger_scheduled_digest_delivery()
    return {key: str(value) for key, value in result.items()}
